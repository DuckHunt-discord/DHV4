...
from utils.models import DiscordChannel, Player, get_from_db, get_player
...
class ShoppingCommands(Cog):
    ...
    async def bullet(self, ctx: MyContext):
        ...
        if db_hunter.bullets >= max_bullets:
            await ctx.reply(
                _("❌ Whoops, you have too many bullets in your weapon already.")
            )
            return False
        ...
        await ctx.reply(
            _(
                "💸 You added a bullet in your weapon. [Bought: -{ITEM_COST} exp, total {db_hunter.experience} exp]",
                db_hunter=db_hunter,
                ITEM_COST=ITEM_COST,
            )
        )
    ...
    async def magazine(self, ctx: MyContext):
        ...
        if db_hunter.magazines >= max_magazines:
            await ctx.reply(
                _(
                    "❌ Whoops, you have too many magazines in your backpack already... Try reloading!"
                )
            )
            return False
        ...
        if db_hunter.bullets <= 0:
            await ctx.reply(
                _(
                    "💸 You added a magazine in your weapon. Time to reload! [Bought: -{ITEM_COST} exp, total {db_hunter.experience} exp]",
                    db_hunter=db_hunter,
                    ITEM_COST=ITEM_COST,
                )
            )
            ...
            await ctx.reply(
                _(
                    "💸 You added a magazine in your weapon. [Bought: -{ITEM_COST} exp, total {db_hunter.experience} exp]",
                    db_hunter=db_hunter,
                    ITEM_COST=ITEM_COST,
                )
            )
    ...
    async def ap(self, ctx: MyContext):
        ...
        if db_hunter.is_powerup_active("ap_ammo"):
            time_delta = get_timedelta(
                db_hunter.active_powerups["ap_ammo"], time.time()
            )

            await ctx.reply(
                _(
                    "❌ Whoops, your gun is already using AP ammo for {time_delta}!",
            ...
            await ctx.reply(
                _(
                    "❌ Your gun is using some even better explosive ammo for {time_delta}!",
                    time_delta=format_timedelta(time_delta, locale=language_code),
                )
            )
            return False
        ...
        await ctx.reply(
            _(
                "💸 You bought some AP ammo. Twice the damage, twice the fun! [Bought: -{ITEM_COST} exp, total {db_hunter.experience} exp]",
                db_hunter=db_hunter,
                ITEM_COST=ITEM_COST,
            )
        )
    ...
    async def explosive(self, ctx: MyContext):
        ...
        if db_hunter.is_powerup_active("explosive_ammo"):
            ...
            await ctx.reply(
                _(
                    "❌ Your gun is already using explosive ammo for {time_delta}!",
                    time_delta=format_timedelta(time_delta, locale=language_code),
                )
            )
            return False
        ...
        await ctx.reply(
            _(
                "💸 You bought some **EXPLOSIVE** ammo. Thrice the damage, that'll be bloody! [Bought: -{ITEM_COST} exp, total {db_hunter.experience} exp]",
                db_hunter=db_hunter,
                ITEM_COST=ITEM_COST,
            )
        )
    ...
    async def weapon(self, ctx: MyContext):
        ...
        if not db_hunter.is_powerup_active("confiscated"):
            await ctx.reply(
                _("❌ Your gun isn't confiscated, why would you need a new one ?")
            )
            return False
        ...
        await ctx.reply(
            _(
                "💸 You bribed the police and bought back your weapon. The fun continues. [Bought: -{ITEM_COST} exp, total {db_hunter.experience} exp]",
                db_hunter=db_hunter,
                ITEM_COST=ITEM_COST,
            ),
            file=f,
    ...
    async def grease(self, ctx: MyContext):
        ...
        await ctx.reply(
            _(
                "💸 You added some grease to your weapon to reduce jamming for a day. [Bought: -{ITEM_COST} exp, total {db_hunter.experience} exp]",
                db_hunter=db_hunter,
                ITEM_COST=ITEM_COST,
            )
        )
    ...
    async def sight(self, ctx: MyContext):
        ...
        if db_hunter.is_powerup_active("sight"):
            await ctx.reply(
                _(
                    "❌ You added a new sight to your weapon recently. "
        ...
        await ctx.reply(
            _(
                "💸 You added a sight to your weapon to improve your accuracy for the next few shots. [Bought: -{ITEM_COST} exp, total {db_hunter.experience} exp]",
                db_hunter=db_hunter,
                ITEM_COST=ITEM_COST,
            )
        )
    ...
    async def detector(self, ctx: MyContext):
        ...
        if db_hunter.is_powerup_active("detector"):
            await ctx.reply(
                _(
                    "❌ You already have that infrared detector on your weapon. "
        ...
        await ctx.reply(
            _(
                "💸 You added an infrared detector to your weapon. If you can't see ducks, you can't shoot them. [Bought: -{ITEM_COST} exp, total {db_hunter.experience} exp]",
                db_hunter=db_hunter,
                ITEM_COST=ITEM_COST,
            )
        )
    ...
    async def silencer(self, ctx: MyContext):
        ...
        if db_hunter.is_powerup_active("silencer"):
            language_code = await ctx.get_language_code()
            time_delta = get_timedelta(
                db_hunter.active_powerups["silencer"], time.time()
            )

            await ctx.reply(
                _(
                    "❌ You already use a silencer. It's still good for {time_delta}, come back then.",
                    time_delta=format_timedelta(time_delta, locale=language_code),
                )
            )
            return False
        ...
        if db_hunter.prestige >= 6:
            await ctx.reply(
                _(
                    "💸 You added a military-grade silencer to your weapon. You seem to know the game well. [Bought: -{ITEM_COST} exp, total {db_hunter.experience} exp]",
                    db_hunter=db_hunter,
                    ITEM_COST=ITEM_COST,
                )
            )
            ...
            await ctx.reply(
                _(
                    "💸 You added a silencer to your weapon. Ducks are still afraid of the noise, but you don't make any. [Bought: -{ITEM_COST} exp, total {db_hunter.experience} exp]",
                    db_hunter=db_hunter,
                    ITEM_COST=ITEM_COST,
                )
            )
    ...
    async def clover(self, ctx: MyContext):
        ...
        if db_hunter.is_powerup_active("clover"):
            if self.bot.current_event == Events.FLORIST:
                ...
                time_delta = get_timedelta(
                    db_hunter.active_powerups["clover"], time.time()
                )
                await ctx.reply(
                    _(
    ...
    async def sunglasses(self, ctx: MyContext):
        ...
        if previously_had:
            await ctx.reply(
                _(
                    "😎 You bought sunglasses, again... What a waste. [Bought: -{ITEM_COST} exp, total {db_hunter.experience} exp]",
                    db_hunter=db_hunter,
                    ITEM_COST=ITEM_COST,
                )
            )
            ...
            await ctx.reply(
                _(
                    "😎 You bought sunglasses. [Bought: -{ITEM_COST} exp, total {db_hunter.experience} exp]",
                    db_hunter=db_hunter,
                    ITEM_COST=ITEM_COST,
                )
            )
    ...
    async def clothes(self, ctx: MyContext):
        ...
        await ctx.reply(
            _(
                "💸 You bought some new clothes. You look very good. Maybe the ducks will like your outfit. [Bought: -{ITEM_COST} exp, total {db_hunter.experience} exp]",
                db_hunter=db_hunter,
                ITEM_COST=ITEM_COST,
            )
        )
    ...
    async def brush(self, ctx: MyContext):
        ...
        await ctx.reply(
            _(
                "💸 You've just cleaned your weapon. Could've just shot once, but heh ¯\\_(ツ)_/¯. [Bought: -{ITEM_COST} exp, total {db_hunter.experience} exp]",
                db_hunter=db_hunter,
                ITEM_COST=ITEM_COST,
            )
        )
    ...
    async def mirror(self, ctx: MyContext, target: discord.Member):
        ...
        if target.id == ctx.author.id:
            ...
            await ctx.reply(
                _(
                    "❌ I don't think {target.mention} can play DuckHunt yet...",
                    target=target,
                )
            )
            return False
        ...
        if stupid:
            await ctx.reply(
                _(
                    "💸 You are redirecting ☀️ sunlight towards {target.mention} eyes 👀 using your mirror. "
                    "That was kinda stupid, since they have sunglasses 😎. [Bought: -{ITEM_COST} exp, total {db_hunter.experience} exp]",
                    db_hunter=db_hunter,
                    ITEM_COST=ITEM_COST,
                    target=target,
                )
            ...
            await ctx.reply(
                _(
                    "💸 You are redirecting ☀️ sunlight towards {target.mention} eyes 👀 using your mirror. [Bought: -{ITEM_COST} exp, total {db_hunter.experience} exp]",
                    db_hunter=db_hunter,
                    ITEM_COST=ITEM_COST,
                    target=target,
                )
    ...
    async def sand(self, ctx: MyContext, target: discord.Member):
        ...
        await ctx.reply(
            _(
                "💸 You threw sand in {target.mention} weapon... Not cool! [Bought: -{ITEM_COST} exp, total {db_hunter.experience} exp]",
                db_hunter=db_hunter,
                ITEM_COST=ITEM_COST,
                target=target,
            )
    ...
    async def bucket(self, ctx: MyContext, target: discord.Member):
        ...
        if target_has_coat:
            await ctx.reply(
                _(
                    "💸 You threw water on {target.mention}... But they have a raincoat on. [Fail: -{ITEM_COST} exp]",
                    ITEM_COST=ITEM_COST,
                    target=target,
                )
            )
            ...
            await ctx.reply(
                _(
                    "💸 You threw some icelandic water on {target.mention}... They can't hunt for **three** hours! [Bought: -{ITEM_COST} exp, total {db_hunter.experience} exp]",
                    db_hunter=db_hunter,
                    ITEM_COST=ITEM_COST,
                    target=target,
                )
            ...
            await ctx.reply(
                _(
                    "💸 You threw water on {target.mention}... They can't hunt for an hour! [Bought: -{ITEM_COST} exp, total {db_hunter.experience} exp]",
                    db_hunter=db_hunter,
                    ITEM_COST=ITEM_COST,
                    target=target,
                )
    ...
    async def sabotage(self, ctx: MyContext, target: discord.Member):
        ...
        try:
            await ctx.author.send(
                _(
                    "💸 You sabotaged {target.mention} weapon... They don't know... yet! [Bought: -{ITEM_COST} exp, total {db_hunter.experience} exp]",
                    db_hunter=db_hunter,
                    ITEM_COST=ITEM_COST,
                    target=target,
                )
            ...
            await ctx.reply(
                _(
                    "I couldn't DM you... Are your DMs blocked ? Anyway, you sabotaged {target.name} weapon... "
                    "They don't know... yet! [Bought: -{ITEM_COST} exp, total {db_hunter.experience} exp]",
                    db_hunter=db_hunter,
                    ITEM_COST=ITEM_COST,
                    target=target,
                )
    ...
    async def mechanical(self, ctx: MyContext):
        ...
        async def spawn():
            ...
            await ducks.MechanicalDuck(
                bot=self.bot, channel=ctx.channel, creator=ctx.author
            ).spawn()
        ...
        try:
            await ctx.author.send(
                _(
                    "💸 You started a mechanical duck on {channel.mention}, it will spawn in 90 seconds. [Bought: -{ITEM_COST} exp, total {db_hunter.experience} exp]",
                    db_hunter=db_hunter,
                    ITEM_COST=ITEM_COST,
                    channel=ctx.channel,
                )
            ...
            await ctx.reply(
                _(
                    "💸 You started a mechanical duck on {channel.mention}, it will spawn in 90 seconds. [Bought: -{ITEM_COST} exp].\n**I couldn't DM you this message**",
                    ITEM_COST=ITEM_COST,
                    channel=ctx.channel,
                )
            )
    ...
    async def license(self, ctx: MyContext):
        ...
        await ctx.reply(
            _(
                "💸 You bought a kill license. Accidental kills are now allowed. "
                "[Bought: -{ITEM_COST} exp, total {db_hunter.experience} exp]",
                db_hunter=db_hunter,
                ITEM_COST=ITEM_COST,
            )
    ...
    async def reloader(self, ctx: MyContext):
        ...
        await ctx.reply(
            _(
                "💸 You bought an auto reloader for your weapon. It'll reload automatically every time it's necessary for a day. "
                "[Bought: -{ITEM_COST} exp, total {db_hunter.experience} exp]",
                db_hunter=db_hunter,
                ITEM_COST=ITEM_COST,
            )
    ...
    async def homing_bullets(self, ctx: MyContext):
        ...
        await ctx.reply(
            _(
                "💸 You are now using the brand new Homing Bullets made by CACAD (the Comitee Against the Comitee Against Ducks) in China. "
                "Try them soon! [Bought: -{ITEM_COST} exp, total {db_hunter.experience} exp]",
                db_hunter=db_hunter,
                ITEM_COST=ITEM_COST,
                channel=ctx.channel,
            )
