...
from utils.models import BotList, DiscordUser, Player, Vote, get_from_db
...
class BotsListVoting(Cog):
        ...
        for bot_list in await self.get_bot_list():
            ...
            if handler_type == "generic":
                ...
                self.bot.logger.error(
                    f"Unknown hook type for {bot_list.name} : {handler_type}"
                )
                continue
    ...
    async def votes_topgg_hook(self, request: web.Request):
        ...
        if auth != bot_list.auth:
            self.bot.logger.warning(
                f"Bad authentification provided to {bot_list.name} API."
            )
            return web.Response(status=401, text="Unauthorized, bad auth")
        ...
        if bot_id != self.bot.user.id:
            self.bot.logger.warning(
                f"Bad bot ID ({bot_id}) provided to {bot_list.name} API."
            )
        ...
        result, message = await self.handle_vote(
            user_id, bot_list, multiplicator=multiplicator, is_test=is_test
        )
    ...
    async def handle_vote(
        ...
        try:
            ...
            self.bot.logger.warning(
                f"Bad user ID provided to votes API {bot_list.name}: {user_id}."
            )
        ...
        try:
            await user.send(
                f"✨ Thanks for voting for DuckHunt on {bot_list.name}! "
    ...
    async def can_vote(
        self, bot_list: BotList, user: discord.User, db_user: DiscordUser
    ):
        ...
        if not bot_list.can_vote:
            ...
            last_vote = (
                await Vote.filter(user=db_user, bot_list=bot_list)
                .order_by("-at")
                .first()
            )
            ...
            if last_vote:
                ...
                if timezone.now() > (
                    last_vote.at + vote_every + datetime.timedelta(minutes=5)
                ):
                    return True
                else:
                    return False
            ...
            try:
                async with self.bot.client_session.get(
                    vote_check_url.format(user=user), timeout=timeout, headers=headers
                ) as resp:
                ...
                self.bot.logger.warning(
                    f"Checking user {user.id} vote on {bot_list.name} -> unavailable ({e})"
                )

                return False  # Can't vote if the bot list is down
            except Exception as e:
                # Defensive catch-all so unexpected errors don't crash the command
                self.bot.logger.error(
                    f"Checking user {user.id} vote on {bot_list.name} -> unexpected error {e}"
                )
                return False

            self.bot.logger.debug(
                f"Checking user {user.id} vote on {bot_list.name} -> {json_resp}"
            )
            ...
            if voted_resp.isdigit():
                ...
                self.bot.logger.warning(
                    f"Unknown response for {bot_list.name} votechecking: {voted_resp}"
                )
                voted = False
    ...
    async def get_votable_lists(
        self, user: discord.User
    ) -> Tuple[List[BotList], List[BotList], List[BotList]]:
        votable_lists = []
        maybe_lists = []
        nope_lists = []
    ...
    async def vote(self, ctx: MyContext):
        ...
        async with ctx.typing():
            ...
            for bot_list in maybe_lists:
                embed.add_field(
                    name=_(
                        "You might be able to vote on {bl_name}", bl_name=bot_list.name
                    ),
                    value=f"[{click_me_to_vote}]({bot_list.vote_url})",
                    inline=True,
                )
    ...
    async def post_stats(self, *args, **kwargs):
        ...
        for bot_list in await self.get_bot_list():
            ...
            if stats_url:
                ...
                try:
                    if method == "POST":
                        resp = await self.bot.client_session.post(
                            stats_url, timeout=timeout, headers=headers, json=post_data
                        )
                    elif method == "PATCH":
                        resp = await self.bot.client_session.patch(
                            stats_url, timeout=timeout, headers=headers, json=post_data
                        )
                    elif method == "None":
                        continue
                        ...
                        self.bot.logger.error(
                            f"Unknown HTTP method to post stats on {bot_list.name}: {method}"
                        )
                    ...
                    if status in [200, 204]:
                        self.bot.logger.debug(
                            f"Pushed stats to {bot_list.name} : resp [{status}] {text}"
                        )
                    else:
                        self.bot.logger.warning(
                            f"Pushed stats to {bot_list.name} : resp [{status}] {text}"
