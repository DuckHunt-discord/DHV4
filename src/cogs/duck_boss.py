...
from discord import Embed, AllowedMentions, Color, Reaction
...
class DuckBoss(Cog):
    ...
    async def create_boss_embed(self, bangs=0, boss_message=None):
        ...
        new_embed = Embed(
            title=choice(["A duck boss is here...", "A wild boss has appeared...", "A boss has spawned...", "KILL THE BOSS !",
    ...
    async def background_loop(self):
        ...
        if not latest_messages:
            ...
            if latest_message.author.id != self.bot.user.id:
                ...
                if latest_message_embed.color == Color.green():
                    boss_message = latest_message
                else:
                    boss_message = None

        if boss_message:
            reaction: Reaction = boss_message.reactions[0]
            ...
            if bangs >= boss_life:
                ...
                new_embed = Embed(
                    title=choice(["The boss was defeated !"]),
                    color=Color.red(),
                ...
                if "boss_llama_Calgeka.png" in str(boss_message.embeds[0].image.url):
                    ...
                    new_embed.set_image(url=choice(["https://cdn.discordapp.com/attachments/795225915248214036/807309301181055056/deadboss_Calgeka.png",
            ...
            if randint(1, self.boss_every_n_minutes) == 1:
    ...
    async def spawn_boss(self):
        ...
        await self.bot.log_to_channel(content=f"<@&{ping_role_id}>", embed=log_embed, allowed_mentions=AllowedMentions(roles=True, users=False, everyone=False))
