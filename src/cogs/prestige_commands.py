...
class PrestigeCommands(Cog):
    ...
    async def info(self, ctx: MyContext):
        ...
        needed_exp = higher_level["expMin"] + (db_hunter.prestige * 100)
        ...
        if current_exp < needed_exp:
            description += _(
                "Prestige will be unlocked when you reach the maximum level. "
            ...
            description += _(
                "Prestige is available for you now ! Go ahead and use `{ctx.prefix}prestige confirm`. "
        ...
        if db_hunter.prestige > 0:
            e.add_field(
                ...
                value=_("You have reached level {level}", level=db_hunter.prestige),
                inline=False,
            )
        else:
            e.add_field(
                name="⚠️",
                value=_(
                    "**Remember**, using prestige means losing your statistics and most of your experience. "
                    "However, you'll get the following items to help you"
                ),
                inline=False,
            )
        ...
        e.add_field(
            name=_("Level 1"),
            value=_("**Unbreakable sunglasses**: Never buy sunglasses again"),
        )
        e.add_field(
            name=_("Level 2"),
            value=_("**Coat colour**: Choose the colour of your coat"),
        )
        e.add_field(
            name=_("Level 3"),
        ...
        e.add_field(
            name=_("Level 6"),
            value=_(
                "**Military grade silencer**: Better silencers that last twice as long"
            ),
        )
        e.add_field(
            name=_("Level 7"),
            value=_("**Permanent licence to kill**: Secret DuckHunt service"),
        )
        e.add_field(
            name=_("Level 8"),
            value=_("**Bigger ammo pack**: Load twice as many bullets in your gun"),
        )
        e.add_field(
            name=_("Level 9"), value=_("**???**: Suggestions are welcome")
        )  # TODO
    ...
    async def confirm(self, ctx: MyContext):
        ...
        if current_exp < needed_exp:
            await ctx.send(
                _(
                    "❌ You haven't unlocked prestige yet. See `{ctx.prefix}prestige info` to learn more."
                )
            )
            return False
        ...
        async with ctx.typing():
            ...
            e = discord.Embed(
                title=_(
                    "Prestige {old_prestige} -> {new_prestige}",
            ...
            e.add_field(
                name=_("✨ New run"),
                value=_(
                    "You'll restart the game with {kept_exp} experience.",
                    kept_exp=kept_exp,
                ),
            )
