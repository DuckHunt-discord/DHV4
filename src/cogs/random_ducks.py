...
class RandomDucks(Cog):
    ...
    async def random_duck(
        self, ctx: MyContext, artist="Calgeka", debug=False, with_background=True
    ):
        """
        ...
        if artist not in ["Calgeka", "Globloxmen"]:
            await ctx.send(
                _("That artist is not known. Choose either Calgeka or Globloxmen.")
            )
            return

        async with ctx.typing():
            file, debug_data = await get_random_duck_file(
                self.bot, artist, with_background
            )
            if debug:
                show_lines = []
