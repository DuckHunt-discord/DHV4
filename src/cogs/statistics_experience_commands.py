...
class TopScoresSource(menus.ListPageSource):
    ...
    async def format_page(self, menu, entries):
        ...
        e.title = self.title
        e.url = (
            f"{self.ctx.bot.config['website_url']}data/channels/{self.ctx.channel.id}"
        )
...
class StatisticsCommands(Cog):
    ...
    async def me(self, ctx: MyContext, target: discord.Member = None):
        ...
        embed.add_field(
            name=_("Bullets"),
            value=f"{db_hunter.bullets}/{level_info['bullets']}",
            inline=True,
        )
        embed.add_field(
            name=_("Magazines"),
            value=f"{db_hunter.magazines}/{level_info['magazines']}",
            inline=True,
        )
        embed.add_field(name=_("Experience"), value=db_hunter.experience, inline=True)
        embed.add_field(
            name=_("Level"),
            value=str(level_info["level"]) + " - " + _(level_info["name"]).title(),
            inline=True,
        )
        embed.add_field(
            name=_("Accuracy"),
            value=_(
                "{level_reliability} % (Real: {real_reliability} %)",
                level_reliability=level_info["accuracy"],
                real_reliability=db_hunter.real_accuracy,
            ),
            inline=True,
        )
        embed.add_field(
        ...
        embed.add_field(
            name=_("Prestige"),
            value=_("Level {level}", level=db_hunter.prestige),
            inline=True,
        )
    ...
    async def shooting_stats(self, ctx: MyContext, target: discord.Member = None):
        ...
        if shots_when_dead:
            embed.add_field(
                name=_("🧠"),
                value=_(
                    "{shots_when_dead} shots attempted when DEAD. "
        ...
        if shots_when_wet:
            embed.add_field(
                name=_("🚰"),
                value=_("{shots} shots attempted when wet.", shots=shots_when_wet),
            )
        ...
        if shots_when_confiscated:
            embed.add_field(
                name=_("⛔️"),
                value=_(
                    "{shots} shots attempted when {target.mention} gun was confiscated.",
                    shots=shots_when_confiscated,
                    target=target,
                ),
            )
        ...
        if shots_when_sabotaged:
            embed.add_field(
                name=_("💥"),
                value=_(
                    "{shots} shots attempted with a sabotaged gun.",
                    shots=shots_when_sabotaged,
                    target=target,
                ),
            )
        ...
        if shots_when_jammed:
            embed.add_field(
                name=_("☁"),
                value=_(
                    "{shots} shots attempted with a jammed weapon.",
                    shots=shots_when_jammed,
                    target=target,
                ),
            )

        shots_with_empty_magazine = shooting_stats.get(
            "shots_with_empty_magazine", None
        )
        if shots_with_empty_magazine:
            embed.add_field(
                name=_("🦉"),
                value=_(
                    "{shots} shots attempted without bullets.",
                    shots=shots_with_empty_magazine,
                    target=target,
                ),
            )
        ...
        if shots_jamming_weapon:
            embed.add_field(
                name=_("💥"),
                value=_(
                    "{shots} shots jamming {target.mention} weapon.",
                    shots=shots_jamming_weapon,
                    target=target,
                ),
            )
        ...
        if bullets_used:
            embed.add_field(
                name=_("🔫"),
                value=_(
                    "{bullets} bullets used in a duck killing madness.",
        ...
        if missed:
            embed.add_field(
                name=_("🎯"),
                value=_("{shots} shots missing a target.", shots=missed, target=target),
            )
        ...
        if killed:
            embed.add_field(
                name=_("☠️"),
                value=_(
                    "{shots} shots killing someone ({murders} were murders).",
                    shots=killed,
                    murders=murders,
                    target=target,
                ),
        ...
        if got_killed:
            embed.add_field(
                name=_("🧟"),
                value=_(
                    "{target.mention} was killed {times} times.",
                    times=got_killed,
                    target=target,
                ),
            )
        ...
        if homing_kills:
            embed.add_field(
                name=_("🏆️"),
                value=_(
                    "{shots} shots using homing bullets.",
                    shots=homing_kills,
                    target=target,
                ),
            )
        ...
        if shots_with_duck:
            embed.add_field(
                name=_("🦆"),
                value=_(
                    "{shots} shots going towards ducks.",
                    shots=shots_with_duck,
                    target=target,
                ),
            )

        shots_stopped_by_detector = shooting_stats.get(
            "shots_stopped_by_detector", None
        )
        if shots_stopped_by_detector:
            embed.add_field(
                name=_("🕵"),
                value=_(
                    "{shots} shots stopped by the infrared detector.",
                    shots=shots_stopped_by_detector,
                    target=target,
                ),
            )
        ...
        if shots_without_ducks:
            embed.add_field(
                name=_("❓"),
                value=_(
                    "{shots} shots without any duck in sight.",
                    shots=shots_without_ducks,
                    target=target,
                ),
            )
    ...
    def add_fields_for_every_duck(self, _, embed, from_dict, not_found_message):
        ...
        for field_name, field_key in fields:
            ...
            if value:
                ...
                embed.add_field(
                    name=field_name, value=str(not_found_message), inline=True
                )
    ...
    async def hugs_stats(self, ctx: MyContext, target: discord.Member = None):
        ...
        if hugged_dead:
            embed.add_field(
                name=_("Hugs when dead"), value=str(hugged_dead), inline=False
            )
    ...
    async def send_exp(self, ctx: MyContext, target: discord.Member, amount: int):
        ...
        if target.id == ctx.author.id:
            ...
            await ctx.reply(
                _(
                    "❌ I'm not sure that {target.mention} can play DuckHunt.",
                    target=target,
                )
            )
            return False
    ...
    async def give_exp(self, ctx: MyContext, target: discord.Member, amount: int):
        ...
        """
        _, db_receiver = await asyncio.gather(
            ctx.get_translate_function(), get_player(target, ctx.channel)
        )
        ...
        await ctx.reply(
            _(
                "💰️ You gave {amount} experience to {reciver.mention}. ",
                amount=amount,
                reciver=target,
            )
        )
    ...
    async def top(self, ctx: MyContext):
        ...
        await show_topscores_pages(
            ctx, _("Top Scores on #{channel}", channel=ctx.channel.name)
        )
    ...
    async def remove_user(
        self, ctx: MyContext, target: Union[discord.Member, discord.User]
    ):
        """
    ...
    @checks.needs_access_level(models.AccessLevel.ADMIN)
    @checks.channel_enabled()
    async def remove_all_scores_stats(
        self, ctx: MyContext, channel_id_to_delete: int = None
    ):
        """
        ...
        async with ctx.typing():
                ...
                if maybe_channel:
                    ...
                    if maybe_db_channel:
                        if maybe_db_channel.guild.discord_id != ctx.guild.id:
                            await ctx.send(
                                _(
                                    "❌ The channel used to exist but on a different server. I can't confirm you have the "
            ...
            await ctx.send(
                _(
                    "Scores and hunters data were removed from the game, but the game wasn't stopped... "
