    ...
    async def carve(
        ...
        async with ctx.typing():
            ...
            for attachment in ctx.message.attachments:
                ...
                if who:
                    image_bytes = await who.display_avatar.replace(
                        format="jpg", size=512
                    ).read()
                else:
                    image_bytes = await ctx.author.display_avatar.replace(
                        format="jpg", size=512
                    ).read()
            ...
            if image_format in {"gif", "webp", "png"}:
                fn = partial(
                    resize_image_gif, image_bytes, width_pct, height_pct, image_format
                )
            else:
            ...
            # prepare the file
            file = discord.File(
                filename="seam_carving." + image_format, fp=final_buffer
            )
