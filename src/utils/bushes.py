...
class BushObject:
    ...
    db = "trash"
    took_message = _(
        "Searching the bushes around the duck, you found... **Some trash lying around**."
    )
    left_message = _(
        "Searching the bushes around the duck, you found... **Some trash lying around**. "
...
class Bullet(BushObject):
    ...
    left_message = _(
        "Searching the bushes around the duck, you found... **A bullet**. "
...
class Magazine(BushObject):
    db = "magazine"
    took_message = _(
        "Searching the bushes around the duck, you found... **A magazine**."
    )
    left_message = _(
        "Searching the bushes around the duck, you found... **A magazine**. "
...
class ExplosiveAmmo(BushObject):
    db = "explosive_ammo"
    took_message = _(
        "Searching the bushes around the duck, you found... **A box of explosive ammo**. 💥"
    )

    async def give(self, db_channel, db_hunter):
        db_hunter.active_powerups["explosive_ammo"] = (
            max(int(time.time()), db_hunter.active_powerups["explosive_ammo"]) + DAY
        )
        return True
...
class PartialExplosiveAmmo(BushObject):
    ...
    took_message = _(
        "Searching the bushes around the duck, you found... **An almost empty box of explosive ammo**."
    )

    async def give(self, db_channel, db_hunter):
        db_hunter.active_powerups["explosive_ammo"] = (
            max(int(time.time()), db_hunter.active_powerups["explosive_ammo"])
            + 6 * HOUR
        )
        return True
...
class Grease(BushObject):
    ...
    async def give(self, db_channel, db_hunter):
        db_hunter.active_powerups["grease"] = (
            max(int(time.time()), db_hunter.active_powerups["grease"]) + 2 * DAY
        )
        return True
...
class Silencer(BushObject):
    db = "silencer"
    took_message = _(
        "Searching the bushes around the duck, you found... **A new silencer**."
    )

    async def give(self, db_channel, db_hunter: Player):
        db_hunter.active_powerups["silencer"] = (
            max(int(time.time()), db_hunter.active_powerups["silencer"]) + DAY
        )
