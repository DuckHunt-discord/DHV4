...
@unique
class Coats(Enum):
    DEFAULT = _("White"), _("A useful coat when it rains.")

    ORANGE = _("Orange"), _(
        "Better visibility: Increase your chance to frighten ducks, and reduce your chance to get shot by another hunter."
    )

    CAMO = _("Camo"), _("Lower visibility: Less chance to frighten ducks.")

    BLUE = _("Blue"), _("Lucky find: Increase your chance to find items in bushes.")

    RED = _("Red"), _(
        "Hungry for blood: Increase your murder skills, reduce murder penalties."
    )

    YELLOW = _("Yellow"), _("Sun powers: Mirrors are less effective against you.")

    DARK_GREEN = _("Dark green"), _(
        "Farming skills: Clovers give you one more experience point."
    )

    BLACK = _("Black"), _("Secret service: Sabotages are cheaper.")

    # LMAO this is useless
    LIGHT_BLUE = _("Light blue"), _("Sea powers: You are immune to water buckets.")
        ...
        try:
            ...
            raise commands.BadArgument(
                _(
                    ...
                    colors=format_list(
                        list(cls.__members__), locale=await ctx.get_language_code()
                    ),
                )
            )
